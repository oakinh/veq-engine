...
class ProjectionTest : public testing::Test {
  protected:
    ...
    veq::SelectedBatch sb0_{ .columns = column_views_, .selection = selected_indexes_, .size = 3 };
...
TEST_F(ProjectionTest, ReturnsMultipleSelectedColumns) {
    p0_.setTargetColumns({ veq::ColumnView{ t0_.id.data() }, veq::ColumnView{ t0_.age.data() },
                           veq::ColumnView{ t0_.occupation_id.data() } });
...
TEST_F(ProjectionTest, PreservesTargetColumnOrder) {
    const veq::ColumnViews target_columns{ veq::ColumnView{ t0_.age.data() },
