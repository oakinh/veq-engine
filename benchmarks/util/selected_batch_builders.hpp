...
template<typename T>
std::vector<OwnedSelectedBatch>
buildOwnedSelectedBatches(const std::vector<ColumnBatch>& column_batches, ColumnView target_column,
    ...
    for (const auto& batch : column_batches) {
        ...
        std::vector<std::size_t> owned_selection(selection.begin(), selection.begin() + size);

        owned_selected_batches.emplace_back(columns, std::move(owned_selection), size);
    }

    return owned_selected_batches;
}

std::vector<SelectedBatch>
convertOwnedSelectedBatches(const std::vector<OwnedSelectedBatch>& owned_batches) {
    std::vector<SelectedBatch> selected_batches{};

    for (const auto& batch : owned_batches) {
        const auto& [columns, owned_selection, size] = batch;

        selected_batches.emplace_back(columns, owned_selection, size);
    }
